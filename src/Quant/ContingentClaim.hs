...
--contingent claims.
newtype ContingentClaim a b = ContingentClaim { unCC :: [CCProcessor a b] }

-- | Contingent claims with one observable.
type ContingentClaim1 = forall a b . Obs1 a => ContingentClaim a b
-- | Contingent claims with two observables.
type ContingentClaim2 = forall a b . Obs2 a => ContingentClaim a b
-- | Contingent claims with three observables.
type ContingentClaim3 = forall a b . Obs3 a => ContingentClaim a b
-- | Contingent claims with four observables.
type ContingentClaim4 = forall a b . Obs4 a => ContingentClaim a b
-- | Contingent claims with four observables.
type ContingentClaim5 = forall a b . Obs5 a => ContingentClaim a b

instance Monoid (ContingentClaim a b) where
...
data CCProcessor a b = CCProcessor  {
    monitorTime      :: Time
  , payoutFunc       :: Maybe [M.Map Time (a b) -> CashFlow b]
}

promote :: Functor a => Int -> [M.Map Time (a Double) -> CashFlow Double] 
                            -> [M.Map Time (a MCVector) -> CashFlow MCVector] 
promote len fs = map promote' fs
  where
    promote' f = f'
      where f' m = CashFlow cfT $ U.fromList $ map (\k -> cfAmount . f $ fmap (fmap (U.!k)) m) [0..len-1]
              where CashFlow cfT _ = f $ fmap (fmap U.head) m 

type CCBuilder w r a = WriterT w (Reader r) a

-- | 'monitor' gets the value of the first observable at a given time.
monitor :: Obs1 a => Time -> CCBuilder (ContingentClaim a Double) (M.Map Time (a Double)) Double
monitor = monitor1

-- | 'monitor1' gets the value of the first observable at a given time.
monitor1 :: Obs1 a => Time -> CCBuilder (ContingentClaim a Double) (M.Map Time (a Double)) Double
monitor1 = monitorGeneric get1

-- | 'monitor2' gets the value of the second observable at a given time.
monitor2 :: Obs2 a => Time -> CCBuilder (ContingentClaim a Double) (M.Map Time (a Double)) Double
monitor2 = monitorGeneric get2

-- | 'monitor3' gets the value of the third observable at a given time.
monitor3 :: Obs3 a => Time -> CCBuilder (ContingentClaim a Double) (M.Map Time (a Double)) Double
monitor3 = monitorGeneric get3

-- | 'monitor4' gets the value of the fourth observable at a given time.
monitor4 :: Obs4 a => Time -> CCBuilder (ContingentClaim a Double) (M.Map Time (a Double)) Double
monitor4 = monitorGeneric get4

-- | 'monitor5' gets the value of the fifth observable at a given time.
monitor5 :: Obs5 a => Time -> CCBuilder (ContingentClaim a Double) (M.Map Time (a Double)) Double
monitor5 = monitorGeneric get5

-- | 'monitorGeneric' applies any getter at a given time.
monitorGeneric :: (a b -> b) -> Time -> CCBuilder (ContingentClaim a b) (M.Map Time (a b)) b
...
specify :: CCBuilder (ContingentClaim a Double) (M.Map Time (a Double)) (CashFlow Double) -> ContingentClaim a Double
...
terminalOnly :: Obs1 a => Time -> (Double -> Double) -> ContingentClaim a Double
...
vanillaOption :: Obs1 a => OptionType -> Double -> Time -> ContingentClaim a Double
...
binaryOption :: Obs1 a => OptionType -> Double -> Double -> Time -> ContingentClaim a Double
...
arithmeticAsianOption :: Obs1 a => OptionType -> Double -> [Time] -> Time -> ContingentClaim a Double
...
-- | Takes an OptionType, a strike, observation times, time to
--maturity and generates an arithmetic Asian option.
geometricAsianOption :: Obs1 a => OptionType -> Double -> [Time] -> Time -> ContingentClaim a Double
...
multiplier :: Double -> ContingentClaim a Double -> ContingentClaim a Double
...
forwardContract :: Obs1 a => Time -> ContingentClaim a Double
...
callSpread :: Obs1 a => Double -> Double -> Time -> ContingentClaim a Double
...
putSpread :: Obs1 a => Double -> Double -> Time -> ContingentClaim a Double
...
straddle :: Obs1 a => Double -> Time -> ContingentClaim a Double
straddle strike t = vanillaOption Put strike t <> vanillaOption Call strike t

-- | Combines two contingent claims into one. 
combine :: ContingentClaim a b -> ContingentClaim a b -> ContingentClaim a b
